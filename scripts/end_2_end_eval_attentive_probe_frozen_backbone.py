...
parser.add_argument(
    "--models_combination",
    type=str,
    default="./scripts/configs/all_tokens_one_layer/model_combinations_all_tokens_last_layer_end2end.txt",
...
if __name__ == "__main__":
    ...
    # TODO: not sure about that, as we optimize the attention dropout during training,
    # so we should not need a validation set
    val_proportion = 0
    ...
    for model_set in model_combinations:
        ...
        if "small" in model_set[0] or "-B-32" in model_set[0]:
            partition = "gpu-2d"
            qos="normal"
        else:
            partition = "gpu-7d"
        ...
        print(f"Running EndToEnd attentive probe for {model_set}")
        for proj_drop, att_drop in hyper_params["attention_dropout"]:
            print(
                f"Running attentive probe for:\n"
                f"{len(model_set)=} models, {model_set[0]}, {model_set[-1]},\n"
                f"{proj_drop=} dropout,\n"
                f"{att_drop=} dropout,\n"
                f"{n_heads=} heads,\n"
                f"{DATASETS=} dataset,\n"
                f"and {dim} dimensions"
            )
            ...
            job_cmd = f"""python src/cli.py --dataset {DATASETS} \
                            ...
                            --freeze_premodel
            """
            # TODO: remove this later, if we have intermediate layers then do fine-tuning of the premodel, otherwise freeze it
            run_job(
                job_name=f"end2end_aat_probe_with_n{len(model_set)}",
                job_cmd=job_cmd,
                partition=partition,
