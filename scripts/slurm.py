...
from scripts.project_location import (
...
def run_job(
    ...
    qos="normal",
    time=None,
    constraint="'[80gb|40gb|h100]'",
):
    if apptainer:
        if copy_sqfs:
            mounts = " -B ".join([BASE_PATH_PROJECT, *ADDITIONAL_MOUNTS_w_copy])
            submit_cmd = f"""
            cp /home/space/datasets-sqfs/imagenet_torchvision.sqfs /tmp/
            apptainer run --nv --writable-tmpfs {mounts}{container_path} /bin/bash -c "cd $PATH_TO_REPO && {job_cmd}"
            """
        else:
            mounts = " -B ".join([BASE_PATH_PROJECT, *ADDITIONAL_MOUNTS_wo_copy])
    ...
    device, partition_time = partition.split("-")

    if device == "gpu":
        slurm_options["gres"] = "gpu:1"
        if partition != "gpu-test":
            slurm_options["constraint"] = constraint
        slurm_options["exclude"] = "head075,head074,head073"
        slurm_options["qos"] = qos

    if time is None:
        print("No time specified, using default time from partition.")
        time_mapping = {
            "test": "00-00:15:00",
            "9m": "00-00:09:00",
            "2h": "00-02:00:00",
        ...
        slurm_options["time"] = time_mapping[partition_time]
    else:
        slurm_options["time"] = time
